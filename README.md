...
The following sections give a detailed example about how to submit a job. You can clone the spark-job repository anywhere; in this documentation, we place spark-job in the home directory for illustration.
...
```shell
$ cd spark-job/submitdir/
# Submit a Spark job with 2 nodes to read data from DAOS and run K-Means example.
$ ../bin/submit-spark.sh -A Aurora_deployment -l walltime=2:00:00 -l select=2 -l filesystems=flare:daos_user -q lustre_scaling $HOME/spark-job/example/kmeans-pyspark.py daos://Intel2/hadoop_fs/HiBench/Kmeans/Input/36000000
# Submit a Spark job with 2 nodes to read data from Lustre Filesystem and run K-Means example.
$ ../bin/submit-spark.sh -A Aurora_deployment -l walltime=2:00:00 -l select=2 -l filesystems=flare:daos_user -q lustre_scaling $HOME/spark-job/example/kmeans-pyspark.py file:///lus/flare/projects/Aurora_deployment/spark/DataRoot/HiBench/Kmeans/Input/36000000
...
```shell
# Submitting job: $HOME/spark-job/example/kmeans-pyspark.py daos://Intel/hadoop_fs2/HiBench/Kmeans/Input/36000000
debug 1286330.aurora-pbs-0001.hostmgmt.cm.aurora.alcf.anl.gov
debug -A Aurora_deployment -l select=2 -l walltime=2:00:00 -l filesystems=flare:daos_user -q lustre_scaling -v SPARKJOB_SCRIPTS_DIR=$HOME/spark-job/bin,SPARKJOB_CONFIG_DIR=/home/damon/spark_work_home,SPARKJOB_INTERACTIVE=0,SPARKJOB_SCRIPTMODE=0,SPARKJOB_OUTPUT_DIR=/home/damon/spark_work_home,SPARKJOB_SEPARATE_MASTER=0,SPARKJOB_OAPML=1,SPARKJOB_DAOS=1,SPARKJOB_ARG=$HOME/spark-job/example/kmeans-pyspark.py^daos://Intel2/hadoop_fs/HiBench/Kmeans/Input/36000000 -o /home/damon/spark_work_home -e /home/damon/spark_work_home $HOME/spark-job/bin/start-spark.sh
# Submitted
...
```shell
# Submitting job: $HOME/spark-job/example/kmeans-pyspark.py daos://Intel2/hadoop_fs/HiBench/Kmeans/Input/36000000
...
```shell
$ ../bin/submit-spark.sh -A Aurora_deployment -l walltime=2:00:00 -l select=2 -l filesystems=flare:daos_user -q lustre_scaling -I
```
...
debug -A Aurora_deployment -l select=2 -l walltime=2:00:00 -l filesystems=flare:daos_user -q lustre_scaling -v SPARKJOB_SCRIPTS_DIR=$HOME/spark-job/bin,SPARKJOB_CONFIG_DIR=/home/damon/spark_work_home,SPARKJOB_INTERACTIVE=1,SPARKJOB_SCRIPTMODE=0,SPARKJOB_OUTPUT_DIR=/home/damon/spark_work_home,SPARKJOB_SEPARATE_MASTER=0,SPARKJOB_OAPML=1,SPARKJOB_DAOS=1,SPARKJOB_ARG= -o /home/damon/spark_work_home -e /home/damon/spark_work_home $HOME/spark-job/bin/start-spark.sh
# Submitted
SPARKJOB_JOBID=67690.amn-0001
need spark job host: 0
Waiting for Spark to launch by checking $HOME/spark-job/submitdir/67690.amn-0001
...
sourced $HOME/spark-job/bin/env_gpu.sh
need spark job host: 1
loading DAOS module
sourced $HOME/spark-job/submitdir/env_aurora.sh
sourced $HOME/spark-job/bin/env_spark_daos.sh
GPU Options: --conf spark.oap.mllib.device=GPU
sourced $HOME/spark-job/submitdir/env_local.sh
```
After entering the interactive interface, you can run command line.
```shell
$ spark-submit $HOME/spark-job/example/kmeans-pyspark.py daos://Intel2/hadoop_fs/HiBench/Kmeans/Input/36000000
```
...
```shell
$ $HOME/spark-job/bin/submit-spark.sh -h
...
#### Bash Mode:
```shell
$ ../bin/submit-spark.sh -A Aurora_deployment -l walltime=2:00:00 -l select=1 -l filesystems=flare:daos_user -q lustre_scaling -s test_script.sh
```
#### Interactive Mode:
```shell
$ ../bin/submit-spark.sh -A Aurora_deployment -l walltime=2:00:00 -l select=1 -l filesystems=flare:daos_user -q lustre_scaling
$ ./test_script.sh
```
